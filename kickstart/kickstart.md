...
```
cat > /var/lib/tftpboot/pxelinux.cfg/ks.cfg << EOT
...
rootpw  --iscrypted $6$0bATj9oiiRo7hXKD$JGvpp9xnLnQ3dg4Wr.YJQTOGHGBjtCQKG7ZYhoRoagHJx2DtdnT6pUd66CN9GqnMqen7yjFzQRkGLaYJp5tRI0
...
[root@pxe ~]# service strat httpd
[root@pxe ~]# elinks http://172.16.10.2/myks.cfg
```

 5) 发布iso ---> http://172.16.10.2/dvd/
```
...
[root@pxe-server ~]# mkdir /var/www/html/rhel7 -p
 ---下载镜像，再挂载
[root@pxe-server ~]# mount -o loop /mnt/rhel7.3/x86_64/isos/rhel-server-7.3-x86_64-dvd.iso /var/www/html/rhel7
...
url --url="http://192.168.0.16/rhel6"
# Root password
...
[root@pxe-server rhel6]# cd isolinux/
